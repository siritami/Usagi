...
object MangaListFilterSerializer : KSerializer<MangaListFilter> {
		...
		decoder.decodeStructure(descriptor) {
			...
			while (true) {
				when (decodeElementIndex(descriptor)) {
					...
					1 -> {
						tags = decodeSerializableElement(descriptor, 1, SetSerializer(MangaTagSerializer))
					}

					2 -> {
						tagsExclude = decodeSerializableElement(descriptor, 2, SetSerializer(MangaTagSerializer))
					}
					...
					5 -> {
						states = decodeSerializableElement(descriptor, 5, SetSerializer(serializer()))
					}

					6 -> {
						contentRating = decodeSerializableElement(descriptor, 6, SetSerializer(serializer()))
					}

					7 -> {
						types = decodeSerializableElement(descriptor, 7, SetSerializer(serializer()))
					}

					8 -> {
						demographics = decodeSerializableElement(descriptor, 8, SetSerializer(serializer()))
					}

					9 -> {
						year = decodeIntElement(descriptor, 9)
					}

					10 -> {
						yearFrom = decodeIntElement(descriptor, 10)
					}

					11 -> {
						yearTo = decodeIntElement(descriptor, 11)
					}
	...
	private object MangaTagSerializer : KSerializer<MangaTag> {
		override val descriptor: SerialDescriptor =
			buildClassSerialDescriptor(MangaTag::class.java.name) {
		...
		override fun deserialize(decoder: Decoder): MangaTag =
			decoder.decodeStructure(descriptor) {
